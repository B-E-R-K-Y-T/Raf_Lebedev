...
CI_DOCKER_IMAGE="${CI_DOCKER_IMAGE:-registry.gitlab.com/reconquista-lebed/shop-with-postgresql}"
...
if [[ "$1 $2" == "docker login" ]]; then
    ...
    docker build --pull --platform amd64 \
        --tag "registry.gitlab.com/reconquista-lebed/shop-with-postgresql:dependency-${CI_DOCKER_TAG}" \
        --cache-from "registry.gitlab.com/reconquista-lebed/shop-with-postgresql:dependency-${CI_DOCKER_TAG}" \
    ...
    docker push "registry.gitlab.com/reconquista-lebed/shop-with-postgresql:dependency-${CI_DOCKER_TAG}" || exit 1
elif [[ "$1 $2" == "docker build" ]]; then
    if [[ "$3" ]]; then
        CI_DOCKER_TAG="$3"
    fi

    docker pull "registry.gitlab.com/reconquista-lebed/shop-with-postgresql:dependency-${CI_DOCKER_TAG}" || true

    docker build --pull --platform amd64 \
        --tag "registry.gitlab.com/reconquista-lebed/shop-with-postgresql:${CI_DOCKER_TAG}" \
        --cache-from "registry.gitlab.com/reconquista-lebed/shop-with-postgresql:dependency-${CI_DOCKER_TAG}" \
        --build-arg DOCKER_DEPENDENCY_IMAGE="${DOCKER_DEPENDENCY_IMAGE:-gradle:7.6.1-jdk11}" \
        --build-arg DOCKER_RUNTIME_IMAGE="${DOCKER_RUNTIME_IMAGE:-openjdk:11-jre-slim}" \
        --build-arg BUILDKIT_INLINE_CACHE=1 \
